...
const Checkout = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const isOnline = effectivePaymentMethod === 'online';

      // 1. Create order
      const order = await createOrder(data, isOnline ? 'pending_payment' : 'new');

      // 2. Create order items
      await createOrderItems(order.id);

      // 3. Mark personal coupon as used
      if (appliedUserCouponId) {
        await markCouponUsed(appliedUserCouponId, order.id);
      }
      ...
      if (isOnline) {
        ...
        if (paymentError || !paymentData?.paymentUrl) {
          ...
          await rollbackOrder(order.id, appliedUserCouponId);
          toast({
            title: "Ошибка оплаты",
        ...
        // Telegram notification sent from yookassa-callback after successful payment
        clearCart();
        window.location.href = paymentData.paymentUrl;
        return;
      }

      // 5. Offline payment — send Telegram notification
      try {
        const paymentLabel = effectivePaymentMethod === 'card' ? 'Картой при получении' : 'Наличными';
